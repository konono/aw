...
if [ -f "$WORKSPACE/devbox.json" ]; then
  ...
  run_as_agent "cd \"$WORKSPACE\" && devbox install"
  run_as_agent "cd \"$WORKSPACE\" && devbox run install 2>/dev/null" || true
...
if run_as_agent 'eval "$(devbox global shellenv 2>/dev/null)"; command -v npx' > /dev/null 2>&1; then
  if [ ! -d /home/agent/.cache/ms-playwright ]; then
    echo "Installing Playwright browsers..."
    run_as_agent 'eval "$(devbox global shellenv 2>/dev/null)"; npx -y playwright install chromium'
  fi
fi
